...
/*   Updated: 2023/02/21 18:16:14 by cfamilar         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*trim_line_and_get_leftover(char *line)
{
	...
	leftover = (char *) malloc (sizeof(char) * (ft_strlen_gnl(line) - i + 1));
	if (!leftover)
		return (0);
	j = 0;
	i++;
...
{
	char		*line;
	static char	*unread_string;

	if (fd < 0 || BUFFER_SIZE <= 0 || fd > FOPEN_MAX || read(fd, 0, 0) < 0)
		return (NULL);
	unread_string = buffer_read(fd, unread_string);
	if (!unread_string)
		return (0);
	line = find_new_line(unread_string);
	unread_string = trim_line_and_get_leftover(unread_string);
	return (line);
}
/*
int	main(void)
